...
class Entry(Thread):
    ...
    def _python(self, arg):
        ...
        process = subprocess.Popen([F'{pycmd}', F'{PATH+arg}'], shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def pinInit(self, pin_home=26, pin_up=19, pin_down=13, pin_check=6):
        ...
        self._pi.callback(pin_up, pigpio.FALLING_EDGE , self.up)
        self._pi.callback(pin_down, pigpio.FALLING_EDGE , self.down)
        self._pi.callback(pin_check, pigpio.FALLING_EDGE , self.check)
